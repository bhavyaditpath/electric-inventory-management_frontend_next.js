...
const PurchasePage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let purchaseIdToUse: number | null = editingPurchase?.id ?? null;

      if (editingPurchase) {
        const updatedPurchase = await purchaseApi.editRecordPurchase(editingPurchase.id.toString(), payload);
        if (!updatedPurchase) {
        ...
        const createdPurchase = (await purchaseApi.recordPurchase(payload)) as any | null;
        if (!createdPurchase || typeof createdPurchase.id !== 'number') {
          showError('Failed to record purchase');
          setLoading(false);
          return;
        }
        purchaseIdToUse = createdPurchase.id;
      ...
      if (!purchaseIdToUse) {
        showError('Failed to determine purchase record');
        setLoading(false);
        return;
      }
        ...
        <div className="bg-[var(--theme-surface)] rounded-lg border border-[var(--theme-border)] shadow-sm">
          ...
          <div className="p-4 sm:p-6">
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-[var(--theme-text)] mb-2">Supplier Name</label>
                <select
                  value={adminUserId ?? ''}
                  onChange={(e) => {
